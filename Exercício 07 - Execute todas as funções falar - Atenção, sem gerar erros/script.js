...
// Forma que eu fiz
function filtrarFuncaoFalar(personagens)   {
...
const personagensFalantes = personagens.filter(filtrarFuncaoFalar);
const falar = personagensFalantes.map(chamarFuncaoFalar);

// Resposta final
personagens.filter(personagem => personagem.falar).map(falador => falador.falar());